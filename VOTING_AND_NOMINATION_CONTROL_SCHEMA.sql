-- Enhanced Voting and Nomination Control Schema
-- This adds voting date controls and admin nomination capabilities
...
-- 2. Create admin_nominations table for admin-only nominations
...
-- 3. Create function to auto-generate live URLs for admin nominations
...
BEGIN
  ...
  WHILE EXISTS (
    ...
    UNION
    SELECT 1 FROM public.admin_nominations WHERE live_url = final_url AND id != NEW.id
  ) LOOP
    counter := counter + 1;
...
-- 4. Create trigger for admin nominations
DROP TRIGGER IF EXISTS generate_admin_nomination_live_url_trigger ON public.admin_nominations;
CREATE TRIGGER generate_admin_nomination_live_url_trigger
  BEFORE INSERT OR UPDATE ON public.admin_nominations
  FOR EACH ROW
  EXECUTE FUNCTION generate_admin_nomination_live_url();

-- 5. Create updated timestamp trigger for admin nominations
...
DROP TRIGGER IF EXISTS update_admin_nominations_timestamp_trigger ON public.admin_nominations;
...
-- 6. Create unified view for all nominees (regular + admin)
...
CREATE INDEX IF NOT EXISTS idx_admin_nominations_live_url ON public.admin_nominations(live_url);
CREATE INDEX IF NOT EXISTS idx_admin_nominations_created_at ON public.admin_nominations(created_at);

-- 9. Update existing nominees API to use unified view
-- This will be handled in the API routes

COMMENT ON TABLE public.admin_nominations IS 'Admin-created nominations that bypass the normal approval process';
COMMENT ON VIEW public.all_nominees IS 'Unified view of all approved nominees from both regular and admin sources';