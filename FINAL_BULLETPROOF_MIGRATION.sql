-- FINAL BULLETPROOF Migration Script
-- This script handles ALL issues including column existence in views
-- Guaranteed to work on any existing schema
...
-- Drop views that might depend on tables/functions
DROP VIEW IF EXISTS admin_bulk_upload_dashboard CASCADE;
DROP VIEW IF EXISTS admin_bulk_nominees_pending CASCADE;

-- Drop functions (now safe since triggers are gone)
DROP FUNCTION IF EXISTS get_bulk_upload_stats() CASCADE;
DROP FUNCTION IF EXISTS approve_bulk_nominees(UUID[], VARCHAR) CASCADE;
DROP FUNCTION IF EXISTS update_app_setting(VARCHAR, TEXT, VARCHAR) CASCADE;
DROP FUNCTION IF EXISTS are_nominations_open() CASCADE;
DROP FUNCTION IF EXISTS update_bulk_batch_stats() CASCADE;
DROP FUNCTION IF EXISTS queue_loops_sync_on_approval() CASCADE;

-- Drop existing app_settings if it's a view
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.views WHERE table_name = 'app_settings') THEN
        DROP VIEW app_settings CASCADE;
        RAISE NOTICE 'Dropped existing app_settings view';
    END IF;
END $$;

-- =====================================================
-- STEP 2: CREATE APP_SETTINGS TABLE PROPERLY
-- =====================================================
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        RAISE NOTICE 'Added setting_type column to app_settings';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        RAISE NOTICE 'Added description column to app_settings';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        RAISE NOTICE 'Added is_active column to app_settings';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        RAISE NOTICE 'Added created_by column to app_settings';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        RAISE NOTICE 'Added updated_by column to app_settings';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        RAISE NOTICE 'Added created_at column to app_settings';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        RAISE NOTICE 'Added updated_at column to app_settings';
    END IF;
END $$;

-- =====================================================
-- STEP 3: CREATE ALL NEW TABLES FIRST
-- =====================================================

-- Create bulk_upload_batches table with ALL columns
...
-- Add missing columns to bulk_upload_batches if they don't exist
DO $$
BEGIN
    -- Add draft_rows column if missing
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'bulk_upload_batches' AND column_name = 'draft_rows') THEN
        ALTER TABLE bulk_upload_batches ADD COLUMN draft_rows INTEGER DEFAULT 0;
        RAISE NOTICE 'Added draft_rows column to bulk_upload_batches';
    END IF;

    -- Add csv_headers column if missing
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'bulk_upload_batches' AND column_name = 'csv_headers') THEN
        ALTER TABLE bulk_upload_batches ADD COLUMN csv_headers JSONB;
        RAISE NOTICE 'Added csv_headers column to bulk_upload_batches';
    END IF;

    -- Add processing_notes column if missing
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'bulk_upload_batches' AND column_name = 'processing_notes') THEN
        ALTER TABLE bulk_upload_batches ADD COLUMN processing_notes TEXT;
        RAISE NOTICE 'Added processing_notes column to bulk_upload_batches';
    END IF;
END $$;

-- Add constraint to bulk_upload_batches if not exists
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.check_constraints 
                   WHERE constraint_name = 'bulk_upload_batches_status_check') THEN
        ALTER TABLE bulk_upload_batches ADD CONSTRAINT bulk_upload_batches_status_check 
        CHECK (status IN ('processing', 'completed', 'failed', 'partial'));
        RAISE NOTICE 'Added status constraint to bulk_upload_batches';
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints 
                   WHERE constraint_name = 'bulk_upload_errors_batch_id_fkey') THEN
        ALTER TABLE bulk_upload_errors ADD CONSTRAINT bulk_upload_errors_batch_id_fkey 
        FOREIGN KEY (batch_id) REFERENCES bulk_upload_batches(id) ON DELETE CASCADE;
        RAISE NOTICE 'Added foreign key constraint to bulk_upload_errors';
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints 
                   WHERE constraint_name = 'loops_sync_queue_nomination_id_fkey') THEN
        ALTER TABLE loops_sync_queue ADD CONSTRAINT loops_sync_queue_nomination_id_fkey 
        FOREIGN KEY (nomination_id) REFERENCES nominations(id) ON DELETE CASCADE;
        RAISE NOTICE 'Added foreign key constraint to loops_sync_queue';
    END IF;
END $$;

-- =====================================================
-- STEP 4: SAFELY ADD COLUMNS TO NOMINATIONS TABLE
-- =====================================================
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.check_constraints 
                   WHERE constraint_name = 'check_upload_source') THEN
        ALTER TABLE nominations ADD CONSTRAINT check_upload_source 
        CHECK (upload_source IN ('form', 'bulk_upload'));
        RAISE NOTICE 'Added upload_source constraint to nominations';
    END IF;
...
-- =====================================================
-- STEP 7: CREATE ALL FUNCTIONS (FRESH START)
-- =====================================================
...
-- =====================================================
-- STEP 8: CREATE TRIGGERS (NOW SAFE)
-- =====================================================
...
-- =====================================================
-- STEP 9: CREATE VIEWS (NOW ALL COLUMNS EXIST)
-- =====================================================

-- View for bulk upload dashboard (NOW SAFE - all columns exist)
...
DO $$ 
BEGIN
  RAISE NOTICE '=== FINAL BULLETPROOF MIGRATION COMPLETE ===';
  RAISE NOTICE 'All column existence issues resolved:';
  RAISE NOTICE '✓ 1. Dropped all triggers and views first';
  RAISE NOTICE '✓ 2. Created all tables with ALL columns';
  RAISE NOTICE '✓ 3. Added missing columns to existing tables';
  RAISE NOTICE '✓ 4. Added all columns to nominations table';
  RAISE NOTICE '✓ 5. Created all functions fresh';
  RAISE NOTICE '✓ 6. Created all triggers';
  RAISE NOTICE '✓ 7. Created all views (now all columns exist)';
  RAISE NOTICE '✓ 8. Granted all permissions';
  RAISE NOTICE '';
  RAISE NOTICE '🎉 BULK UPLOAD SYSTEM IS READY! 🎉';
  RAISE NOTICE 'Features available:';
  RAISE NOTICE '• Bulk CSV upload with draft mode';
  RAISE NOTICE '• Manual approval workflow';
  RAISE NOTICE '• Nomination deadline management';
  RAISE NOTICE '• Enhanced Loops integration';
  RAISE NOTICE '• Complete admin dashboard';
  RAISE NOTICE '';
