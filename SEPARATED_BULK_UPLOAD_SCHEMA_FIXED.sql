-- Separated Bulk Upload System Schema (FIXED)
-- This creates tables for managing person and company bulk uploads separately

-- Create loops_user_groups table first (needed for foreign key references)
...
-- Create a view for draft nominations ready for approval (FIXED: using 'state' instead of 'status')
...
-- Function to update batch statistics (FIXED: using 'state' instead of 'status')
