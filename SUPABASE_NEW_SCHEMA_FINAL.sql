-- WSA 2026 Complete New Schema for Individual Nominee Pages
...
DROP VIEW IF EXISTS public.voting_stats CASCADE;
...
CREATE TABLE public.nominees (
  ...
  CONSTRAINT nominee_person_required CHECK (
    type != 'person' OR (firstname IS NOT NULL AND lastname IS NOT NULL)
  ),
...
-- Public nominees view (approved nominations only) - THIS IS CRITICAL FOR INDIVIDUAL PAGES
...
-- Grant read permissions for anon role on views
GRANT SELECT ON public.public_nominees TO anon;
GRANT SELECT ON public.voting_stats TO anon;

-- Success message
DO $$
BEGIN
  RAISE NOTICE 'WSA 2026 New Schema created successfully!';
  RAISE NOTICE 'Tables created: nominators, nominees, nominations, voters, votes, hubspot_outbox';
  RAISE NOTICE 'Views created: public_nominees, admin_nominations, voting_stats';
  RAISE NOTICE 'Individual nominee pages will now work correctly.';
END $$;