-- Fix for admin nomination system - Create missing tables
...
CREATE POLICY IF NOT EXISTS "Allow admin write access to category_groups" ON category_groups FOR ALL USING (true);
CREATE POLICY IF NOT EXISTS "Allow admin write access to subcategories" ON subcategories FOR ALL USING (true);
CREATE POLICY IF NOT EXISTS "Allow admin write access to settings" ON settings FOR ALL USING (true);
