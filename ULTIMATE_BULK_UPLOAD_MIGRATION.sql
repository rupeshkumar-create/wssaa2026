-- ULTIMATE Bulk Upload Migration Script
-- This script handles ALL conflicts including existing functions
-- Run this to completely set up bulk upload functionality
...
DROP FUNCTION IF EXISTS get_bulk_upload_stats();
DROP FUNCTION IF EXISTS approve_bulk_nominees(UUID[], VARCHAR);
DROP FUNCTION IF EXISTS update_app_setting(VARCHAR, TEXT, VARCHAR);
DROP FUNCTION IF EXISTS are_nominations_open();
DROP FUNCTION IF EXISTS update_bulk_batch_stats();

-- =====================================================
-- STEP 2: CHECK AND HANDLE EXISTING APP_SETTINGS
-- =====================================================

-- Check if app_settings exists as a view and drop it if needed
DO $$
BEGIN
    -- Check if app_settings exists as a view
    ...
    -- Check if app_settings exists as a table
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'app_settings' AND table_type = 'BASE TABLE') THEN
        RAISE NOTICE 'app_settings table already exists - will verify structure';
    ELSE
...
-- =====================================================
-- STEP 3: CREATE APP_SETTINGS TABLE PROPERLY
-- =====================================================
...
-- =====================================================
-- STEP 8: CREATE FUNCTIONS (FRESH START)
-- =====================================================

-- Function to update bulk batch statistics
CREATE FUNCTION update_bulk_batch_stats()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE FUNCTION are_nominations_open()
RETURNS BOOLEAN AS $$
DECLARE
...
-- Function to get bulk upload statistics (NEW SIGNATURE)
CREATE FUNCTION get_bulk_upload_stats()
RETURNS TABLE (
...
CREATE FUNCTION approve_bulk_nominees(nominee_ids UUID[], approved_by_user VARCHAR(100))
RETURNS TABLE (
...
CREATE FUNCTION update_app_setting(
...
CREATE VIEW admin_bulk_upload_dashboard AS
SELECT
    b.id as batch_id,
    b.filename,
    b.total_rows,
...
CREATE VIEW admin_bulk_nominees_pending AS
SELECT
...
DO $$ 
BEGIN
  RAISE NOTICE '=== ULTIMATE BULK UPLOAD MIGRATION COMPLETE ===';
  RAISE NOTICE 'All conflicts resolved:';
  RAISE NOTICE '✓ Existing functions dropped and recreated';
  RAISE NOTICE '✓ app_settings table created properly';
  RAISE NOTICE '✓ All columns added safely to nominations table';
  RAISE NOTICE '✓ All new tables created with proper constraints';
  RAISE NOTICE '✓ All functions and triggers created fresh';
  RAISE NOTICE '✓ All views created for admin dashboard';
  RAISE NOTICE '✓ All permissions granted';
  RAISE NOTICE '';
  RAISE NOTICE 'Ready for bulk upload functionality!';
  RAISE NOTICE 'No more conflicts - everything is clean!';
END $$;