...
export async function GET() {
  try {
    ...
    if (error && error.code !== 'PGRST116') {
      console.error('Summit banner fetch error:', error);
      return NextResponse.json({ 
        success: false, 
        error: 'Failed to fetch summit banner' 
      }, { status: 500 });
    }
    ...
    console.error('Summit banner API error:', error);
    return NextResponse.json({ 
      success: false, 
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Deactivate existing banners first
    await supabase
      .from('summit_banners')
